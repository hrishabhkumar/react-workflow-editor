...
const Editor = (props, ref) => {
    ...
    const handlePaste = (event) => {
        ...
        if (text.startsWith('react-workflow-editor:')) {
          ...
          || (currentNode.type === 'switch' && node.type === 'case')
            ...
            const { data: data1, copyDetail } = getNewFlowByPaste({
                config: data,
                sourceNode: node,
                containerId,
                containerIndex: containerIndex1,
            });
            const sourceParentNode = getParentNodeById(data1, node.id);
            const sourceIndex = sourceParentNode.children.findIndex(
                x => x.id === node.id,
            );
            if (props.onChange && typeof props.onChange === 'function') {
    ...
    const handleDragOver = ({ currentContainerId, currentContainerIndex }) => (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleDragLeave = ({ currentContainerId, currentContainerIndex }) => (e) => {
        e.stopPropagation();
        if (currentContainerId === containerId && currentContainerIndex === containerIndex) {
            setContainerId(null);
            setContainerIndex(null);
    ...
    const renderLine = ({
        currentContainerId, currentContainerIndex, hasArrow, style,
    }) => {
        ...
        return (
            <div
                ...
                style={style}
                onDragOver={handleDragOver({ currentContainerId, currentContainerIndex })}
                onDragLeave={handleDragLeave({ currentContainerId, currentContainerIndex })}
                onDrop={handleDrop}
            >
                <div className="line" />
                <div className="rect" />
